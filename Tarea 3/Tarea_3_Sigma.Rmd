...
## 1. El objetivo de este ejercicio es calibrar el m ́etodo de ADA para esta Tabla de Datos. Aqu ́ı interesa predecir en la variable tipo. Usando los paquetes snow y traineR programe en paralelo 5 Validaciones Cruzadas con 10 grupos calibrando el modelo de acuerdo con los tres tipos de algoritmos que permite, discrete, real y gentle. Para medir la calidad de m ́etodo sume la cantidad de 1’s detectados en los diferentes grupos. Luego grafique las 5 iteraciones para los tres algoritmos en el mismo gra ́fico. ¿Se puede determinar con claridad cua ́l algoritmo es el mejor? Para generar los modelos predictivos use las siguientes instrucciones:
...
for(i in 1:cantidad.validacion.cruzada) {
  ...
  for(k in 1:cantidad.grupos) {
    ...
    resultado <- clusterApply(clp, algoritmos, function(pkernels) {
      MC <- ejecutar.prediccion(datos, Potability ~ .,muestra, train.ada, iter = 80 , nu = 1, type = pkernels)
...
resumen.lineas(resultado.si, "Cantidad de Sí")
```

## 2. Utilizando las matrices de confusión calculadas en el ejercicio anterior, en lugar de sumar la cantidad de 1’s, promedie los errores globales cometidos en los diferentes grupos (folds). Luego grafique las 5 iteraciones para los tres algoritmos en el mismo gráfico. ¿Se puede determinar con claridad cuál algoritmo es el mejor?

```{r, fig.width=6, fig.height=3}
resultado.global <- data.frame(
  "rep" = 1:cantidad.validacion.cruzada,
   "discrete"     =  (1- sapply(MCs.discrete, precision.global))*100,
  "real"     = (1-sapply(MCs.real, precision.global))*100,
  "gentle" = (1-sapply(MCs.gentle, precision.global))*100
  )
...
## 3. ¿Cuál algoritmo usaría con base en la información obtenida en los dos ejercicios anteriores?

Usaría `discrete` debido a que fue el algoritmo con menor varianza para ambos ejercicios.


# Ejercicio 2: __[30 puntos]__ Para esta pregunta usaremos nuevamente los datos `potabilidad_V2.csv`.
...
for(i in 1:cantidad.validacion.cruzada) {
  ...
  for(k in 1:cantidad.grupos) {
    ...
    resultado <- clusterApply(clp, algoritmos, function(pkernels) {
      MC <- ejecutar.prediccion(datos, Potability ~ .,muestra, train.knn, kernel = pkernels)
      valores <- list(Tipo = pkernels, Resultado = MC)
      valores
    })
...
## 2. Utilizando las matrices de confusión calculadas en el ejercicio anterior, en lugar de sumar la cantidad de 1’s, promedie los errores globales cometidos en los diferentes grupos (folds). Luego grafique las 5 iteraciones para todos los algoritmos en el mismo gráfico. ¿Se puede determinar con claridad cuál algoritmo es el mejor?

```{r, fig.width=6, fig.height=3}
resultado.global <- data.frame(
  "rep" = 1:cantidad.validacion.cruzada,
   "rectangular"     = (1-sapply(MCs.rectangular, precision.global))*100,
  "triangular"     = (1-sapply(MCs.triangular, precision.global))*100,
  "epanechnikov" =(1-sapply(MCs.epanechnikov, precision.global))*100,
  "biweight"     = (1-sapply(MCs.biweight,precision.global))*100,
  "triweight"     = (1-sapply(MCs.triweight, precision.global))*100,
  "cos" = (1-sapply(MCs.cos, precision.global))*100,
  "inv"     = (1-sapply(MCs.inv, precision.global))*100,
  "gaussian"     = (1-sapply(MCs.gaussian, precision.global))*100,
  "optimal" = (1-sapply(MCs.optimal, precision.global))*100)
...
ejecutar.prediccion.particular <- function(datos, formula, muestra, metodo) {
  ...
  if(metodo == "train.knn"){return(ejecutar.prediccion(datos, formula, muestra, metodo, kmax = 37, kernel = "rectangular"))}
...
## 2. Utilizando las matrices de confusión calculadas en el ejercicio anterior, en lugar de sumar la cantidad de 1’s, promedie los errores globales cometidos en los diferentes grupos (folds). Luego grafique las 5 iteraciones para todos los métodos vistos en el curso en el mismo gráfico. ¿Se puede determinar con claridad cuál algoritmo es el mejor?
...
## 3. ¿Cuál método usaría con base en la información obtenida en los dos ejercicios anteriores?
